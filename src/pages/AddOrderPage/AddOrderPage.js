...
class AddOrderPage extends Component {
  ...
  handleDeleteFriends = deletedId => {
    this.setState({
      friends: this.state.friends.filter(friend => friend._id !== deletedId)
    });
    this.setState({ friend_deleted: deletedId });
  };
  handleDeleteGroups = deletedId => {
    this.setState({
      groups: this.state.groups.filter(group => group._id !== deletedId)
    });
  ...
  render() {
      ...
      friends.map(friend => (
        <tr key={friend._id}>
          ...
          <td>
            <Button
              color="danger"
              onClick={() => this.handleDeleteFriends(friend._id)}
            >
              Delete
            </Button>
          </td>
    ...
    const groupsView = groups.length ? (
      groups.map(group => (
        <tr key={group._id}>
          <td>{group.name}</td>
          <td>
            <Button
              color="danger"
              onClick={() => this.handleDeleteGroups(group._id)}
            >
              Delete
            </Button>
          </td>
    ...
    return ((
      <div className="AddOrderPage">
        <AddOrderComponent
          group_deleted_id={this.state.group_deleted}
